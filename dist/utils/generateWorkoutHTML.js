export function generateWorkoutHTML(workoutPlan) {
    ...
    const { workout_plan, meal_plan, caloric_deficit_strategy, budget_plan, summary, } = workoutPlan;
    // Generate Workout Plan Table for each muscle group
    let exercisesHTML = '';
    Object.keys(workout_plan.muscle_groups).forEach((muscleGroup) => {
        const exercises = workout_plan.muscle_groups[muscleGroup].exercises;
        exercisesHTML += `
      <h3>${muscleGroup.charAt(0).toUpperCase() + muscleGroup.slice(1)} Exercises</h3>
      <table>
        <tr>
          <th>Exercise</th>
          <th>Sets</th>
    ...
    // Generate Meal Plan
    let mealPlanHTML = '';
    Object.keys(meal_plan).forEach((mealType) => {
    ...
    let budgetMealsHTML = '';
    ['protein', 'carbs', 'fats'].forEach((category) => {
